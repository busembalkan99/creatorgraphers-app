...
await A.waitForTimeout(1300);
bekle('4: geri alınca liste boşalıyor', (await A.locator('.cikan').count()) === 0 && ((await admin.from('diskalifiye').select('kare')).data ?? []).length === 0);
await git(A, 'asama');
bekle('4: geri alınca gelmeyen kutusu yeniden çıkıyor', await var_(A, 'Gelmeyen 1 kişi 1 kare yüklemiş'));
await A.getByRole('button', { name: 'Karelerini çıkar' }).click();
await A.waitForTimeout(1300);
